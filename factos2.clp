(deffacts usuarios
   (usuario (nombre "Josefina") (ocupacion "limpieza")(acceso "2"))   
   (usuario (nombre "Javier") (ocupacion "tecnico")(acceso "2"))
   (usuario (nombre "David") (ocupacion "scrum master")(acceso "3"))
   (usuario (nombre "Daniel") (ocupacion "oficinista")(acceso "1"))
   (usuario (nombre "Tomas") (ocupacion "terrorista")(acceso "0")))
   
   
(deffacts zonas
   (zona (nombre "area descanso")(acceso "1")(contenido "no sensible"))
   (zona (nombre "entrada")(acceso "1")(contenido "no sensible")) 
   (zona (nombre "limpieza")(acceso "2")(contenido "sensible"))
   (zona (nombre "oficinas") (acceso "1")(contenido "sensible"))
   (zona (nombre "servidores") (acceso "2")(contenido "sensible"))
   (zona (nombre "comunicaciones") (acceso "2")(contenido "sensible"))
   (zona (nombre "area control") (acceso "3")(contenido "sensible")))
   



